...
@connect(({ outStorageAdd, loading }) => ({
  outStorageAdd,
  loading: loading.models.outStorageAdd,
}))
@Form.create()
class CreateForm extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'outStorageAdd/fetchProduct',
    });
  }
  ...
  renderProductContent = () => {
    ...
    const data = this.props.outStorageAdd.product.list;
    const { form } = this.props;
    return (
...
class PickForm extends PureComponent {
  ...
  componentDidMount() {
    const { dispatch } = this.props;
    dispatch({
      type: 'outStorageAdd/fetchCustomer',
    });
  }
  ...
  renderContent = () => {
    ...
    const data = this.props.outStorageAdd.customer.list;
    const { form } = this.props;
    return (
      <div className={styles.tableListForm}>
...
class TableList extends PureComponent {
  ...
  componentDidMount() {
    const { dispatch } = this.props;
    dispatch({
      type: 'outStorageAdd/fetchType',
    });
    dispatch({
      type: 'outStorageAdd/fetchWarehouse',
    });
  }
    ...
    return (
      <Form layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
            ...
            <FormItem label="单据类型">
              {getFieldDecorator('outType')(
                <Select placeholder="请选择" style={{ width: '100%' }}>
                  {
                    (this.props.outStorageAdd.type.list || []).map((item,index)=>{
          ...
          <Col md={8} sm={24}>
            <FormItem label="客户编号">
              {getFieldDecorator('customerNumber')(<Input placeholder="点击以选择" onClick={() => this.handlePickModalVisible(true)} readOnly
               style={{backgroundColor:'#f5f5f5'}}/>)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="客户名称">
              {getFieldDecorator('customerName')(<Input disabled />)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="客户联系人">
              {getFieldDecorator('customerContact')(<Input disabled/>)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="客户电话">
              {getFieldDecorator('customerPhone')(<Input disabled />)}
            </FormItem>
          </Col>
          <FormItem>
            {getFieldDecorator('customerId')(<Input hidden/>)}
          </FormItem>
          <Col md={8} sm={24}>
            <FormItem label="制单时间">
          ...
          <Col md={8} sm={24}>
            <FormItem label="出库仓库">
              {getFieldDecorator('storageId')(
                <Select placeholder="请选择" style={{ width: '100%' }}>
                  {
                    (this.props.outStorageAdd.warehouse.list || []).map((item,index)=>{
                      return <Option key={index} value={item.id}>{item.name}</Option>
                    })
                  }
                </Select>
